...
int do_list(int controlfd, int datafd, char *input){
    ...
    if (!(in = popen(str, "r"))) {
    	sprintf(sendline, "Requested action aborted. Local error in processing\n");
    	write(controlfd, sendline, strlen(sendline));
        return -1;
    }
...
int do_retr(int controlfd, int datafd, char *input){
	...
	if(get_filename(input, filename) > 0){
		...
		sprintf(sendline, "Requested file action not taken.\nFilename Not Detected\n");
    	write(controlfd, sendline, strlen(sendline));
		return -1;
	}
...
int main(int argc, char **argv){
	...
	if(argc != 4){
		...
		printf("Usage: ./forked <cantidad> <ftp-root> <port>\n");
		exit(-1);
	}
